...
import { accentHex, accentOnHex, accentSoftHex, accentSoftBorderHex } from "@/lib/accents";
      ...
      <section className="relative bg-porcelain pt-10 md:pt-16 pb-12 md:pb-16 border-b border-line">
        <div className="mx-auto max-w-[1600px] px-5 md:px-10">
          <div className="flex items-center justify-between border-b border-line pb-4 mb-10 md:mb-16">
            <span className="mono-label text-ink/70">Chapter 02 · Expertise</span>
            <span className="folio text-ink/65">003 / 018</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            <div className="md:col-span-3">
              <FadeUp>
                <p className="mono-label text-ink/70 mb-4">Practice Areas</p>
      ...
      <section className="bg-porcelain py-16 md:py-24">
        <div className="mx-auto max-w-[1600px] px-5 md:px-10">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-10 pb-6 border-b border-line">
            <span className="mono-label text-ink/70">Legend</span>
            {practiceAreas.map((area) => (
              <span key={area.slug} className="flex items-center gap-2">
                ...
                <span className="mono-label text-ink/75">{area.title}</span>
              </span>
            ))}
          </div>
          ...
          <div className="border-t border-line">
            {practiceAreas.map((area, i) => {
              ...
              const softHex = accentSoftHex[area.accent];
              const softBorder = accentSoftBorderHex[area.accent];
                  ...
                  <div className="border-b border-line">
                    ...
                    >
                      ...
                      <span
                        ...
                        aria-hidden="true"
                      />
                      <span className="relative col-span-2 md:col-span-1 mono-num text-sm transition-colors" style={{ color: isOpen ? onHex : "#5B6475" }}>
                        {area.index}
                      </span>
                      ...
                      <span className="relative col-span-3 md:col-span-4 text-[0.8rem] md:text-sm hidden sm:block transition-colors" style={{ color: isOpen ? onHex : "#303A50" }}>
                        {area.short}
                      </span>
                      <span className="relative col-span-3 md:col-span-2 flex items-center justify-end gap-3">
                        <span className="mono-label transition-colors" style={{ color: isOpen ? onHex : "#5B6475" }}>
                          {isOpen ? "Close" : "Expand"}
                        </span>
                        <svg
                          className={`h-5 w-5 transition-all duration-300 ${isOpen ? "rotate-45" : ""}`}
                          style={{ color: isOpen ? onHex : "#5B6475" }}
                    ...
                    {/* Expandable content — EXPLICIT dark text on soft-blue surface.
                        Does NOT inherit white text from the active blue header. */}
                    {isOpen && (
                      <div
                        className="relative grid grid-cols-12 gap-4 pb-8 md:pb-10 px-4 md:px-6 pt-6"
                        style={{
                          background: softHex,
                          color: "#273047",
                      ...
                      >
                        ...
                        <div className="col-span-10 md:col-span-7">
                          <p className="lead max-w-xl" style={{ color: "#273047" }}>
                            {area.overview}
                          </p>
                          <div className="mt-6 space-y-px">
                            {area.services.map((s, si) => (
                              <div key={s} className="flex items-baseline gap-4 py-3 border-t" style={{ borderColor: softBorder }}>
                                <span className="mono-num text-[0.7rem]" style={{ color: "#46536B" }}>
                                  {String(si + 1).padStart(2, "0")}
                                </span>
                                <span className="font-display text-lg md:text-xl" style={{ color: "#0B1020" }}>
                                  {s}
                                </span>
                              </div>
                            ))}
